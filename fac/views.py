...
class ClienteView(LoginRequiredMixin,SinPrivilegios, generic.ListView):
...
class ClienteNew(LoginRequiredMixin,VistaBaseCreate):
    model=Cliente
...
class ClienteEdit(LoginRequiredMixin,VistaBaseEdit):
    model=Cliente
    template_name = "fac/cliente_form.html"
    form_class = ClienteForm
    success_url = reverse_lazy("fac:cliente_list")
...
def cliente_inactivar(request, id):
    pro = Cliente.objects.filter(pk=id).first()

    contexto = {}
    template_name = "fac/inactivar_prv.html"
...
class FacturaView(LoginRequiredMixin,SinPrivilegios,generic.ListView):
    model = FacturaEnc
...
def facturas(request,id=None):
    ...
    if request.method == "POST":
        ...
        precioo = request.POST.get("precio")
